...
<script>
    ...
    var untukTabelDt = function() {
        dataTable = $('#datatable-server').DataTable({
            ...
            pageLength: 10,
            ajax: '<?= admin_url() ?>kriteria/list_dt',
            columns: [{
                    title: 'No.',
                    data: null,
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        <div class="button-icon-btn button-icon-btn-cl">
                            <button type="button" id="btn-upd" data-id="` + full.id_kriteria + `" class="btn btn-info btn-sm waves-effect" data-toggle="modal" data-target="#modal-add-upd"><i class="fa fa-pencil"></i>&nbsp;Ubah</button>&nbsp;
                            <button type="button" id="btn-del" data-id="` + full.id_kriteria + `" class="btn btn-warning btn-sm waves-effect"><i class="fa fa-trash"></i>&nbsp;Hapus</button>
                        </div>
                    `;
                    },
                },
        ...
        $(document).on('click', '#btn-del', function() {
            ...
            swal({
                ...
                if (del) {
                    $.ajax({
                        type: "post",
                        url: "<?= admin_url() ?>kriteria/destroy",
                        dataType: 'json',
                        data: {
                            id: ini.data('id'),
                            '<?= $this->security->get_csrf_token_name() ?>': csrf.val(),
